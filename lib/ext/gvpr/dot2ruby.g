...
BEG_G {
  ...
  g_direct = isDirect($);
  if( g_direct == 0 ) {
    printf( "graph_%s = GraphViz.graph( \"%s\"", $.name, $.name );
  } else {
    printf( "graph_%s = GraphViz.digraph( \"%s\"", $.name, $.name );
  }
  // Strict
  g_strict = isStrict($);
  if( g_strict != 0 ) {
  ...
  while( subgraph != NULL ) {
    ...
    printf ( "  graph_%s = graph_%s.add_graph( \"%s\" )\n", subgraph.name, pgraph.name, subgraph.name );
...
N {
  ...
  printf( "  node_%s = graph_%s.add_node( \"%s\"", $.name, ofgraph.name, $.name );
...
E {
  ...
  printf( "  graph_%s.add_edge( \"%s\", \"%s\"", ofgraph.name, $.tail.name, $.head.name );
